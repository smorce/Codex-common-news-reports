...
### 絶対ルール（ぼんやり要約の禁止）
- 「洗練された」「おしゃれ」「トレンドを取り入れた」「こなれ感」「上品」「抜け感」など、**根拠のない形容詞だけ**の文は書かない。使う場合は必ず同じ文に **品目・色・シルエット・素材・仕立て・合わせる別アイテム・画面位置・時刻/区間** のうち 2 つ以上を含める。
...
- 各配列の 1 行は「観察した事実 → それがコーデ上どう効くか」の順に書く。事実がない行は削除する。
- 画面から見える情報と、セリフ・テロップ由来の情報を混ぜない。根拠欄に「画面」「セリフ」「テロップ」「タイトル」のどれかを明記する。
...
6. **不足情報の明示**: ブランド名・価格・素材・サイズ感・季節などが見えない/言われない場合は、補わず `unknowns` に列挙する。

### 出力形式（キー名は厳守）
<<<JSON_OUTPUT
{
  "summary": "動画全体を450〜650字で。最初の2文で誰/どの品目/どんな色や形が中心かを書く。抽象語だけの段落にしない",
  "key_points": [
    "各項目は一文で、品目名＋色/形/素材/合わせ方のうち2要素以上＋根拠（画面/セリフ/テロップ/タイトル）を含める。合計8〜10個"
  ],
  "item_material_stitching_coordination": [
    "品目｜色/柄｜形/丈/量感｜素材/縫製（不明なら素材の明示なし）｜合わせた別アイテム｜根拠、の順で1行にする。5〜12行"
  ],
  "evidence_notes": [
    "根拠種別（画面/セリフ/テロップ/タイトル）: 観察できた具体事実を短く書く。最低5個"
  ],
  "unknowns": [
    "動画から確認できないがレポート品質に影響する情報（例: 素材名、価格、ブランド、正確な色名、サイズ）"
  ],
  ...
  "conclusion": "動画の核心を1〜2文。品目名と根拠を入れ、抽象語禁止ルールを守る",
  "recommended_action": "視聴者が明日試せる一手。手持ち服の色/形と、合わせる動画内アイテムの色/形を指定する。曖昧な「参考に」は禁止"
}
JSON_OUTPUT>>>
"""
...
def format_json_summary_to_markdown(data: dict) -> str:
    ...
    if evidence_notes is not None and not (isinstance(evidence_notes, list) and len(evidence_notes) == 0):
        lines.extend(["#### 観察根拠", ""])
        if isinstance(evidence_notes, list):
            for note in evidence_notes:
                lines.append(f"- {note}")
        else:
            lines.append(f"- {evidence_notes}")
        lines.append("")

    unknowns = data.get("unknowns")
    if unknowns is not None and not (isinstance(unknowns, list) and len(unknowns) == 0):
        lines.extend(["#### 不明点", ""])
...
服装・小物がある場合は色・シルエット・丈感・素材らしさ・合わせた別アイテム・画面位置をできる限り入れる。
「おしゃれ」「きれいめ」「カジュアル」だけの箇条書きは禁止。そう書く場合は、同じ文に根拠となる品目名と見た目を入れる。
見えない素材名・ブランド名・価格・正確な色名は推測せず、不明として扱う。
出力は JSON だけにしてください。

<<<JSON_OUTPUT
{{
  "window_summary": "この区間で何が起きているかを 140〜240 字で。人物/品目/色/形/合わせた別アイテム/画面上の変化を含める",
  "important_points": [
    "根拠種別（画面/セリフ/テロップ）＋具体事実（品名・色・形・素材の言及など）。各1文",
    "重要点2",
    "重要点3"
  ],
  "visual_changes": [
    "画面や場面の変化（人物・陳列・テロップ・服の見え方の切り替えなど）を具体語で",
    "画面や場面の変化2"
  ],
  "unknowns": [
    "この区間だけでは確認できない素材名・価格・ブランド・サイズ感など"
  ]
}}
JSON_OUTPUT>>>
"""
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "区間メモにある根拠種別（画面/セリフ/テロップ）を保持し、最終 JSON の evidence_notes に移してください。",
            "区間メモの unknowns は、解決できない限り最終 JSON の unknowns に残してください。",
            "動画に無い情報の補完や一般論への逃げ（例：単に「トレンド」「おしゃれ」）は禁止。",
            "最終出力前に、各 key_points に品目名と具体的な見た目が入っているかを自己点検し、不足する行は書き直してください。",
